                ...
                <div class="h-100">
                    <div class="card">
                        <div class="card-header align-items-center d-flex">
                            <h4 class="card-title mb-0 flex-grow-1">Danh sách chức vụ</h4>
                            <a href="{{route('chucvu.create')}}" class="btn btn-soft-success material-shadow-none">
                                {{-- <i class="?act=form-them-danh-muc="ri-add-circle-line align-middle me-1"></i> --}}
                                Thêm chức vụ
                            </a>
                        </div><!-- end card header -->
                        <form action="{{route('chucvu.index')}}" method="GET" class="d-flex mb-4">
                                ...
                                <div class="table-responsive">
                                    <table class="table table-striped table-nowrap align-middle mb-0">
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <a href="{{route('chucvu.edit',$cv->id)}}"class="btn btn-warning">Sửa</a>
                                                <form action="{{route('chucvu.destroy',$cv->id)}}" method="POST" onsubmit="return confirm('Bạn có muốn xoá ko ?') ">
                                                    @csrf
                                                    @method('DELETE')
                                    ...
                                    <div class="mt-3">
                                        {{$chucvu->links("pagination::bootstrap-5")}}
                                    </div>
                                </div>
                            </div>
