                ...
                <div class="h-100">
                    <div class="card">
                        <div class="card-header align-items-center d-flex">
                            ...
                            <a href="{{route('sanpham.create')}}" class="btn btn-soft-success material-shadow-none">
                                {{-- <i class="?act=form-them-danh-muc="ri-add-circle-line align-middle me-1"></i> --}}
                                Thêm sản phẩm
                            </a>
                        </div><!-- end card header -->
                        <form action="{{route('sanpham.index')}}" method="GET" class="d-flex mb-4">
                        ...
                        <div class="card-body">
                            <div class="live-preview">
                                <div class="table-responsive">
                                    <table class="table table-striped table-nowrap align-middle mb-0">
                                        <thead>
                                            <tr>
                                                ...
                                                <th scope="col">Giá Khuyến Mại</th>
                                        ...
                                        @foreach ($listSP as $index =>$sp)
                                        <tr>
                                            <td>{{$index + 1}}</td>
                                            <td>{{$sp->ten_san_pham}}</td>
                                            <td>{{$sp->ma_san_pham}}</td>
                                            <td><img src="{{ Storage::url($sp->anh_san_pham) }}" class="img-thumbnail" alt="hinh anh" width="100px"></td>
                                            <td>{{$sp->gia}}</td>
                                            ...
                                            <td>{{ $sp->created_at }}</td>
                                            <td>
                                                <a href="{{route('sanpham.create',$sp->id)}}"class="btn btn-primary">Xem</a>
                                                <a href="{{route('sanpham.edit',$sp->id) }}"class="btn btn-warning">Sửa</a>
                                                <form action="{{route('sanpham.destroy',$sp->id) }}" method="POST" onsubmit="return confirm('Bạn có muốn xoá ko ?') ">
                                                    @csrf
                                                    @method('DELETE')
                                    ...
                                    <div class="mt-3">
                                        {{$listSP->links("pagination::bootstrap-5")}}
                                    </div>
                                </div>
                            </div>
